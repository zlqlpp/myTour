...
public class Exportmathod{
	...
	public  String exportall(String USERPATH,String LINEMAIN,String LINENAME,List<?> list){
		...
		Exportcom.cleanfile(setallpath,new SimpleDateFormat("yyyyMMdd").format(date));

		try {
			File setFile = new File(setallname);
			
			WritableWorkbook wbook = Workbook.createWorkbook(setFile); // 建立excel文件   
	        String tmptitle = LINEMAINs[1]; // 标题   
	        WritableSheet wsheet = wbook.createSheet(LINEMAINs[1], 0); // sheet名称  
