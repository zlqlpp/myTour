		...
		var bro=$.browser;
			$(document).ready(function(){
				...
				if('${EMS_USER.rulLevel}' == '0' || '${EMS_USER.rulLevel}' == '2'){
					...
					$("#PROVINCE_NAMES").val('');
					$("#thead_CLFWTDB_SHSX").show();
					$("#td_ghxuanzhe").show();
				}else{
					$("#thead_CLFWTDB_SHSX").hide();
					$("#td_ghxuanzhe").hide();
				}
			...
			function ajaxQueryPage(pageNo){
				...
				var url = '${ctx}/clfw/b09r02clfwquery!queryclfwtdbsh.action';
			    var params = {
		  ...
		  function shclfwtdb(){
				...
				var tCLFWTDB_SHSX=$("input[name='CLFWTDB_SHSX'][checked]").val();
				$.prompt('该操作将审核揽投部频次配置信息！',{
					buttons: {'确定': true, '取消': false },
					callback: function(v){
						if(v){
							var params = {
									CLFWTDB_SEQID:tTOTAL_ALL_VALUE,
									CLFWTDB_SHSX:tCLFWTDB_SHSX
							};
							var url = '${ctx}/clfw/b09cud02clfwconfig!shclfwtdb.action';
							if(!(bro.msie && bro.version >=8)){
								$.blockUI({ message: '<img src="${ctx}/widgets/jquery/blockui/images/loadingAnimation.gif" />',css:{width:'200', border:'0'}});
							}else{
								$("#nodata").show();
								...
								<td>
									...
									<div id="tablelistdiv">
										<table class="listtable" width="1300"
											...
											<thead id="thead_CLFWTDB_SHSX">
												<th align="left"  colspan="12">
													审核操作：
													<input type="radio" name="CLFWTDB_SHSX" id="CLFWTDB_SHSX" value="1"  checked="checked">通过
													<input type="radio" name="CLFWTDB_SHSX" id="CLFWTDB_SHSX" value="2">拒绝
													<a href='#' onClick="shclfwtdb();">&nbsp;审核配置</a>
												</th>
											</thead>
											<thead>
