	...
	<head>
		...
		<script type="text/javascript">
			...
			function ajaxAddUpAlert(tflag){
				...
				var tCLFW_HBXXGL_ZBS=$('#CLFW_HBXXGL_ZBS').val();
				if(tCLFW_HBXXGL_ZBS==''){
					$.prompt('请填写总包数',{buttons: {'确定': true}});
					return false;
				}
				
				var tCLFW_HBXXGL_ZL=$('#CLFW_HBXXGL_ZL').val();
				if(tCLFW_HBXXGL_ZL==''){
					$.prompt('请填写重量',{buttons: {'确定': true}});
					return false;
				}

				var tCLFW_HBXXGL_LDLSH=$('#CLFW_HBXXGL_LDLSH').val();
				if(tCLFW_HBXXGL_LDLSH==''){
					$.prompt('请填写路单流水号',{buttons: {'确定': true}});
					return false;
				}
			...
			function ajaxAddUp(flag){
				...
				var tCLFW_HBXXGL_LDLSH=$('#CLFW_HBXXGL_LDLSH').val();
				if(tCLFW_HBXXGL_LDLSH.length>80){
					$.prompt('路单流水号大于80',{buttons: {'确定': true}});
					return false;
				}
				...
				var regEx = /[^0-9]+/gi;
				if(tCLFW_HBXXGL_ZBS.length >0 && regEx.test(tCLFW_HBXXGL_ZBS.replace('.',''))){
					$.prompt('请填入正确总包数',{buttons: {'确定': true}});
					return false;
				}
				if(tCLFW_HBXXGL_ZL.length >0 && regEx.test(tCLFW_HBXXGL_ZL.replace('.',''))){
					$.prompt('请填入正确重量',{buttons: {'确定': true}});
					return false;
				}
				...
				var params = {
						...
						CLFW_HBXXGL_ZBS:tCLFW_HBXXGL_ZBS,
						CLFW_HBXXGL_ZL:tCLFW_HBXXGL_ZL,
						CLFW_HBXXGL_SFJID:tCOUNTY_ID_SFJ,
						CLFW_HBXXGL_ZDJID:tCOUNTY_ID_ZDJ,
						CLFW_HBXXGL_SEQID:tCLFW_HBXXGL_SEQID,
						CLFW_HBXXGL_LDLSH:tCLFW_HBXXGL_LDLSH
	            };
				if(flag=='Add'){
					var url = '${ctx}/clfw/b09cud02clfwconfig!addclfwhkwhcg.action';
				}else{
					var url = '${ctx}/clfw/b09cud02clfwconfig!upclfwhkwhcg.action';
				}
									...
									<div id="querydiv">
										<table class="listtable" id="viewquery_table"  width="100%">
											<tr>
												<td class="listtablehead"  colspan="4">
													<font id="fliphelpcon" color="red">
														* 可以在本界面,连续维护数据
														<br>* 【出港日期】字数要小于8字,【航班号】字数要小于10字,【路单流水号】字数要小于80字
											...
											<tr>
												...
												<td class="js_left_txt" id="td_privince_content" colspan="3">
												<input type="text" name="CLFW_HBXXGL_ZBS" style="width: 300px; height:20px;" id="CLFW_HBXXGL_ZBS" class="ac_input" size="300" value="${clfwBean.CLFW_HBXXGL_ZBS}" onFocus="this.select();"/>
												</td>
											</tr>
											<tr>
												<td class="listtablehead" width="100">
												...
												<td class="js_left_txt" id="td_privince_content" colspan="3">
												<input type="text" name="CLFW_HBXXGL_ZL" style="width: 300px; height:20px;" id="CLFW_HBXXGL_ZL" class="ac_input" size="300" value="${clfwBean.CLFW_HBXXGL_ZL}" onFocus="this.select();"/>
												</td>
											</tr>
											<tr>
												<td class="listtablehead" width="100">
												...
												<td class="js_left_txt" id="td_privince_content" colspan="3">
												<input type="text" name="CLFW_HBXXGL_LDLSH" style="width: 300px; height:20px;" id="CLFW_HBXXGL_LDLSH" class="ac_input" size="300" value="${clfwBean.CLFW_HBXXGL_LDLSH}" onFocus="this.select();"/>
												</td>
											</tr>
