		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				if('${FLAG}' == 'UPDATE'){
					...
					$("#tr_STRT_ID").hide();
					if('${organizationBean.NUM_FLAG}'!=''){
					$("input[name='NUM_FLAG'][value=${organizationBean.NUM_FLAG}]").attr("checked",'checked');
					}
					shName('noabbr');
					shNUM_FLAG('noabbr');
				}else{
					...
					$("#upquery").hide();
					$("#addquery").show();
					if('${organizationBean.DIST_CD}' != ''){
						$("#COUNTY_NAMES").append("<option value='${organizationBean.DIST_CD}'>${organizationBean.DIST_CD}</option>"); 
						$("#tr_PROVINCE_NAME").hide();
						$("#tr_TOTAL_NAME").show();
						viewDT();
					}else{
					...
					if('${organizationBean.STRT_ID}' != ''){
						$("#STRT_ID").append("<option value='${organizationBean.STRT_ID}'>${organizationBean.STRT_ID}</option>"); 
						$("#tr_STRT_ID").hide();
					}else{
...
//				}
			...
			function callbackDT(data){
				...
				if(d.length>0){
					for(var i=0;i<d.length;i++){
						if('${organizationBean.DT_PK_CODE}' != '' && '${organizationBean.DT_PK_CODE}' == d[i].DT_PK_CODE){
			...
			function callbackDM(data){
				...
				if(d.length>0){
					for(var i=0;i<d.length;i++){
						if('${organizationBean.DM_PK_CODE}' != '' && '${organizationBean.DM_PK_CODE}' == d[i].DM_PK_CODE){
			...
			function callbackPG(data){
				...
				if(d.length>0){
					for(var i=0;i<d.length;i++){
						if('${organizationBean.PG_PK_CODE}' != '' && '${organizationBean.PG_PK_CODE}' == d[i].PG_PK_CODE){
			...
			function callbackqzuit(data){
				...
				if(d.length>0){
					...
					for(var i=0;i<d.length;i++){
						if('${organizationBean.PREFIX}' != '' && '${organizationBean.PREFIX}' == d[i].FIX){
			...
			function callbackhzuit(data){
				...
				if(d.length>0){
					...
					for(var i=0;i<d.length;i++){
						if('${organizationBean.SUFFIX}' != '' && '${organizationBean.SUFFIX}' == d[i].FIX){
			...
			function ajaxAddUp(flag){
				...
				if(flag=='Add'){
					var url = '${ctx}/address/b04cud02addrconfig!addorganization.action';
				}else{
					var url = '${ctx}/address/b04cud02addrconfig!uporganization.action';
				}
									...
									<div id="querydiv">
										<table class="listtable" id="viewquery_table"  width="100%">
											...
											<tr id="tr_DORPLT_ID">
												...
												<td class="js_left_txt" id="td_DORPLT_ID" colspan="3">
													...
													<tr id="tdSTRT2_NAME">
														...
														<td class="js_left_txt" id="td_city_content" colspan="3">
															<input type="text" name="START_NUM" style="width: 200px; height:20px;" id="START_NUM" class="ac_input" size="300"  value="${organizationBean.START_NUM}" onFocus="this.select();" onKeyUp="shNUM_FLAG('abbr')"/><font color="#FF0000">&nbsp;*</font>
														</td>
													</tr>
													<tr  id="tr_END_NUM"  style="display:none">
														...
														<td class="js_left_txt" id="td_city_content" colspan="3">
															<input type="text" name="END_NUM" style="width: 200px; height:20px;" id="END_NUM" class="ac_input" size="300"  value="${organizationBean.END_NUM}" onFocus="this.select();" onChange="shNUM_FLAG('abbr')"/><font color="#FF0000">&nbsp;*</font>
														</td>
													</tr>
													<tr id="tdSTRT4_NAME">
														<td class="listtablehead">
											...
											<tr style="display:none">
												...
												<td class="js_left_txt" id="td_privince_content" colspan="3">
												<input type="text" name="RSDNBLDG_NAME" style="width: 300px; height:20px;" id="RSDNBLDG_NAME" class="ac_input" size="300" value="${organizationBean.RSDNBLDG_NAME}" onFocus="this.select();"/>
												</td>
											</tr>
											<tr>
												<td class="listtablehead" width="80">
