	...
	<head>
		...
		var tpageNo=1;
			...
			function ajaxQueryADN(tSSLX){
				if(tSSLX.indexOf("vbl2sr")>-1){
					$("#xqmxt").text("查询中...");
				}else if(tSSLX.indexOf("vbl3sr")>-1){
					$("#qtcy").text("查询中...");
				}
			...
			function callbackajaxQueryADN(data){
				...
				var str = d.split('#');
				if(str[1].indexOf("vbl2sr")>-1){
					$("#xqmxt").text("");
					$("#xqmxt").text(str[2]);
				}else if(str[1].indexOf("vbl3sr")>-1){
					$("#qtcy").text("");
					$("#qtcy").text(str[2]);
				}
			...
			function ajaxQueryPage(pageNo){
				...
				var url = '${ctx}/address/b04r02addrquery!querybldgrsdnsluan.action';
			    var params = {
								PROVINCE_ID:tPROVINCE_ID,
								CITY_ID:tCITY_ID,
								COUNTY_ID:tCOUNTY_ID,
			...
			function addquerytable(data){
				...
				if(resultdata.length>0){
					for(var i=0;i<resultdata.length;i++){
						...
						tr+="<a href='#' onClick='fnBldgrsdnsluan(" + resultdata[i].DIST_CD + "," + resultdata[i].RSDNBLDG_ID + ");'>忽略</a> <a href='#' onClick='upBldgrsdns(" + resultdata[i].DIST_CD + "," + resultdata[i].RSDNBLDG_ID + ");'>修改</a> <a href='#' onClick='delBldgrsdnsluan(" + resultdata[i].STRT_ID + "," + resultdata[i].DIST_CD + "," + resultdata[i].RSDNBLDG_ID + "," + '"' + resultdata[i].RSDNBLDG_NAME + '"' + ");'>删除</a></td>";	
			...
			function upBldgrsdns(tDIST_CD,tRSDNBLDG_ID){
				value = window.showModalDialog("${ctx}/address/b04cud01addrview!luanbldgrsdns.action?FLAG=UPDATE&DIST_CD=" + tDIST_CD + "&RSDNBLDG_ID=" + tRSDNBLDG_ID,"Code","dialogWidth:600px;dialogHeight:600px;resizable=1,scrollbars=auto");
				backfresh(value);
			}
			function delBldgrsdnsluan(tSTRT_ID,tDIST_CD,tRSDNBLDG_ID,tRSDNBLDG_NAME){
				$.prompt('该操作将删除此小区！确认执行操作？',{
					...
					callback: function(v){
						if(v){
							...
							var url = '${ctx}/address/b04cud02addrconfig!delbldgrsdnsluan.action';
							if(!(bro.msie && bro.version >=8)){
								$.blockUI({ message: '<img src="${ctx}/widgets/jquery/blockui/images/loadingAnimation.gif" />',css:{width:'200', border:'0'}});
							}else{
								$("#nodata").show();
			...
			function fnBldgrsdnsluan(tDIST_CD,tRSDNBLDG_ID){
				$.prompt('该操作将忽略此机构！确认执行操作？',{
					...
					callback: function(v){
						if(v){
							var params = {
									DIST_CD:tDIST_CD,
									TOTAL_ALL_VALUE:tRSDNBLDG_ID
							};
							var url = '${ctx}/address/b04cud02addrconfig!fnbldgrsdnsluan.action';
							if(!(bro.msie && bro.version >=8)){
								$.blockUI({ message: '<img src="${ctx}/widgets/jquery/blockui/images/loadingAnimation.gif" />',css:{width:'200', border:'0'}});
							}else{
								$("#nodata").show();
									...
									<div id="querydiv">
										<table class="listtable" id="viewquery_table"  width="900">
											...
											<tr id="viewquery_table">
												...
												<td class="js_left_txt" id="td_tpdistrict_content" colspan="3">
													<input type="radio" name="IS_CHAXUNLX" id="IS_CHAXUNLX" value="0" checked="checked">小区名相同<label id="xqmxt" style="color:#FF0000"></label>
