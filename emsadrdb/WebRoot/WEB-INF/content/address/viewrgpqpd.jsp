	...
	<head>
		...
		var bro=$.browser;
			...
			function ajaxQueryPage(pageNo){
				...
				var tPAIXU = $("#PAIXU0").val() + ',' + $("#PAIXU1").val() + ',' + $("#PAIXU2").val() + ',' + $("#PAIXU3").val() + ',' + $("#PAIXU4").val();
				...
				var url = '${ctx}/address/b04r02addrquery!queryrgpqpd.action';
			    var params = {
			...
			function yqls(){
				...
				var sj = myDate.getFullYear() + '-' + tMonth + '-' + myDate.getDate() + ' ' + '00';
			...
			function uprgpqpd(){
				...
				var tPK_REMARK=$("#PK_REMARK").val();
				if(tDT_PK_CODE.length<1 && tPK_REMARK.length<1){
					$.prompt('请选择配置信息',{buttons: {'确定': true}});
					return false;
				}

				var tISWHQUERY = $('#ISWHQUERY').val();
				if(tISWHQUERY.length<1){
					tISWHQUERY = 1;
				}
					...
					callback: function(v){
						if(v){
							var params = {
									...
									ISWHQUERY:tISWHQUERY
							};
							var url = '${ctx}/address/b04cud02addrconfig!uprgpqpd.action';
							if(!(bro.msie && bro.version >=8)){
								$.blockUI({ message: '<img src="${ctx}/widgets/jquery/blockui/images/loadingAnimation.gif" />',css:{width:'200', border:'0'}});
							}else{
								$("#nodata").show();
			...
			function tssq(){
				...
				var tPROVINCE_NAME=$("#PROVINCE_NAMESG").find("option:selected").text(); 
				if(isMunicipalities(tPROVINCE_NAME)){
					if($("#PROVINCE_NAMESG").val() != null){
						tDIST_CD=$("#PROVINCE_NAMESG").val();
						tNEWCITYCODE=$("#PROVINCE_NAMESG").val();
					}
				}
				if($("#CITY_NAMESG").val() != null && $("#CITY_NAMESG").val().length>0){
					tDIST_CD=$("#CITY_NAMESG").val();
					tNEWCITYCODE=$("#CITY_NAMESG").val();
				}
				if($("#COUNTY_NAMESG").val() != null && $("#COUNTY_NAMESG").val().length>0){
					tDIST_CD=$("#COUNTY_NAMESG").val();
				}
				if(tDIST_CD.length<1){
					$.prompt('请选择行政区',{buttons: {'确定': true}});
					return false;
				}
					...
					callback: function(v){
						if(v){
							var params = {
									TOTAL_ALL_VALUE:tTOTAL_ALL_VALUE,
									DIST_CD:tDIST_CD,
									NEWCITYCODE:tNEWCITYCODE,
									ISWHQUERY:tISWHQUERY
							};
							var url = '${ctx}/address/b04cud02addrconfig!tssq.action';
							if(!(bro.msie && bro.version >=8)){
								$.blockUI({ message: '<img src="${ctx}/widgets/jquery/blockui/images/loadingAnimation.gif" />',css:{width:'200', border:'0'}});
							}else{
								$("#nodata").show();
										...
										<table class="listtable" id="viewquery_table"  width="900">
											<tr>
												...
												<td class="listtablehead" id="td_tpdistrict_content">
													...
													<input type="radio" name="ISSR_FLAG" id="ISSR_FLAG" value="0">未匹配投递段
												</td>
											...
											<tr>
												...
												<td class="listtablehead" id="td_privince_content">
													...
													<input type="text" name="ITEM_DATESS" style="width: 200px; height:20px;" id="ITEM_DATESS" class="ac_input" size="300"  onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH'})"/>
												 	-->
													<input readonly type="text" name="ITEM_DATESS" style="width: 200px; height:20px;" id="ITEM_DATESS" class="ac_input" size="300"/>
												</td>
												<td class="listtablehead">
													收寄结束时间:
												</td>
												<td class="listtablehead">
													<input type="text" name="ITEM_DATESE" style="width: 200px; height:20px;" id="ITEM_DATESE" class="ac_input" size="300"  onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH'})"/>
												</td>
											</tr>
											...
											<tr  id="gaojitr1" style="display:none">
												...
												<td class="listtablehead" id="td_tpdistrict_content">
													...
													<input type="radio" name="ISDT" id="ISDT" value="1">存在
													<input type="radio" name="ISDT" id="ISDT" value="0">不存在
												</td>
												<td class="listtablehead">
													人工匹配投递段:
												</td>
												<td class="listtablehead" id="td_tpdistrict_content">
													...
													<input type="radio" name="ISPG" id="ISPG" value="1">存在
													<input type="radio" name="ISPG" id="ISPG" value="0">不存在
												</td>
											</tr>
											<tr id="gaojitr2" style="display:none">
												...
												<td class="listtablehead" id="td_tpdistrict_content">
													<input type="radio" name="IS_DISTRI" id="IS_DISTRI" value="">全部
													<input type="radio" name="IS_DISTRI" id="IS_DISTRI" value="1" checked="checked">参与
													<input type="radio" name="IS_DISTRI" id="IS_DISTRI" value="0">不参与
												</td>
										<td class="listtablehead">
													有/无地址:
												</td>
												<td class="listtablehead" id="td_tpdistrict_content" colspan="3">
													<input type="radio" name="IS_YOUDIZHI" id="IS_YOUDIZHI" value="">全部
													<input type="radio" name="IS_YOUDIZHI" id="IS_YOUDIZHI" value="1" checked="checked">有
													<input type="radio" name="IS_YOUDIZHI" id="IS_YOUDIZHI" value="0">没有
												</td>
											</tr>
											<tr id="gaojitr3" style="display:none">
												...
												<td class="listtablehead" id="td_tpdistrict_content" colspan="3">
													...
													<input type="radio" name="ISWH" id="ISWH" value="2">已处理(备份)
													<input type="radio" name="ISWH" id="ISWH" value="0" checked="checked">未处理
												</td>
											</tr>
											<tr id="gaojitr4" style="display:none">
												<td class="listtablehead">
													排序:
												</td>
												<td class="listtablehead" id="td_tpdistrict_content"  colspan="5">
													<select class="simple" id="PAIXU0"  style="width:100">
													<option value="a.COUNT_NUM desc" selected="selected">频率</option>
													<option value="a.IS_DISTRI desc">集散</option>
													<option value="substr(a.collect_date,0,8) desc">收寄日期</option>
													<option value="a.rec_alladdr">地址</option>
													<option value="a.POSTDISTTYPE desc">处理类型</option>
													</select>
													<select class="simple" id="PAIXU1"  style="width:100">
													<option value="a.COUNT_NUM desc">频率</option>
													<option value="a.IS_DISTRI desc" selected="selected">集散</option>
													<option value="substr(a.collect_date,0,8) desc">收寄日期</option>
													<option value="a.rec_alladdr">地址</option>
													<option value="a.POSTDISTTYPE desc">处理类型</option>
													</select>
													<select class="simple" id="PAIXU2"  style="width:100">
													<option value="a.COUNT_NUM desc">频率</option>
													<option value="a.IS_DISTRI desc">集散</option>
													<option value="substr(a.collect_date,0,8) desc" selected="selected">收寄日期</option>
													<option value="a.rec_alladdr">地址</option>
													<option value="a.POSTDISTTYPE desc">处理类型</option>
													</select>
													<select class="simple" id="PAIXU3"  style="width:100">
													<option value="a.COUNT_NUM desc">频率</option>
													<option value="a.IS_DISTRI desc">集散</option>
													<option value="substr(a.collect_date,0,8) desc">收寄日期</option>
													<option value="a.rec_alladdr" selected="selected">地址</option>
													<option value="a.POSTDISTTYPE desc">处理类型</option>
													</select>
													<select class="simple" id="PAIXU4"  style="width:100">
													<option value="a.COUNT_NUM desc">频率</option>
													<option value="a.IS_DISTRI desc">集散</option>
													<option value="substr(a.collect_date,0,8) desc">收寄日期</option>
													<option value="a.rec_alladdr">地址</option>
													<option value="a.POSTDISTTYPE desc" selected="selected">处理类型</option>
											...
											<tr>
												<td class="listtablehead"  colspan="4" align="right">
													...
													<input style="display:none" id="yqls" type="button" value="历史" onClick="yqls()" />
												</td>
											</tr>
										</table>
									</div>
										...
										<table class="listtable" width="1500"
											...
											<thead>
												...
												<th align="left"  colspan="10">
													...
													&nbsp;备注：
													<select class="simple" id="PK_REMARK"  style="width:100">
													<option value="">请选择</option>
													<option value="2">地址无效/错误</option>
													<option value="3">地址不详</option>
													<option value="4">地址录入不规范</option>
													<option value="5">地址正确</option>
											...
											<thead>
												...
												<th align="left"  colspan="10">
													<s:select theme="simple" list="provincesg" id="PROVINCE_NAMESG" listKey="DISTRICT_CODE" listValue="PROVINCE_NAME" onchange="viewcityg()"></s:select>
													<select class="simple" id="CITY_NAMESG" onChange="viewcountyg()"  style="width:100"></select>
													<select class="simple" id="COUNTY_NAMESG" style="width:100"></select>
													<a href='#' onClick="tssq();">&nbsp;提交</a>
												</th>
											</thead>
											<thead>
