	...
	<head>
		...
		<script type="text/javascript">
			$(document).ready(function(){	
				//alert('${EMS_USER.usDistrictOffice}');
				/*if('${FLAG}' == 'UPDATE'){
					$("#PROVINCE_NAMES").val('${bldgrsdnsBean.DIST_CD}'.substr(0,2) + '0000');
					$("#tr_PROVINCE_NAME").hide();
					viewcity();
					$("#upquery").show();
					$("#addquery").hide();
					$("#tr_TOTAL_NAME").show();
					$("#STRT_ID").append("<option value='${bldgrsdnsBean.STRT_ID}'>${bldgrsdnsBean.TOTAL_STREET_NAME}</option>"); 
					$("#tr_STRT_ID").hide();
					if('${bldgrsdnsBean.NUM_FLAG}'!=''){
					$("input[name='NUM_FLAG'][value=${bldgrsdnsBean.NUM_FLAG}]").attr("checked",'checked');
					}
					shNUM_FLAG('noabbr');
				}else{
					$('#SUFFIX').val("号");
					$("#upquery").hide();
					$("#addquery").show();
					if('${bldgrsdnsBean.DIST_CD}' != ''){
						$("#COUNTY_NAMES").append("<option value='${bldgrsdnsBean.DIST_CD}'>${bldgrsdnsBean.DIST_CD}</option>"); 
						$("#tr_PROVINCE_NAME").hide();
						$("#tr_TOTAL_NAME").show();
						viewDT();
					}else{
					...
					if('${bldgrsdnsBean.STRT_ID}' != ''){
						$("#STRT_ID").append("<option value='${bldgrsdnsBean.STRT_ID}'>${bldgrsdnsBean.STRT_ID}</option>"); 
						$("#tr_STRT_ID").hide();
					}else{
			...
			function callbackcity(data){
				...
				if(isMunicipalities(tPROVINCE_NAME)){
			    	...
			    	for(var i=0;i<d.length;i++){
						if('${bldgrsdnsBean.DIST_CD}' != '' && '${bldgrsdnsBean.DIST_CD}' == d[i].DISTRICT_CODE){
					...
					$('#CITY_NAMES option').remove();
					for(var i=0;i<d.length;i++){
						if('${bldgrsdnsBean.DIST_CD}' != '' && '${bldgrsdnsBean.DIST_CD}'.substr(0,4) + '00' == d[i].DISTRICT_CODE){
...
//				}
			...
			function callbackDT(data){
				...
				if(d.length>0){
					for(var i=0;i<d.length;i++){
						if('${bldgrsdnsBean.DT_PK_CODE}' != '' && '${bldgrsdnsBean.DT_PK_CODE}' == d[i].DT_PK_CODE){
			...
			function callbackDM(data){
				...
				if(d.length>0){
					for(var i=0;i<d.length;i++){
						if('${bldgrsdnsBean.DM_PK_CODE}' != '' && '${bldgrsdnsBean.DM_PK_CODE}' == d[i].DM_PK_CODE){
			...
			function callbackPG(data){
				...
				if(d.length>0){
					for(var i=0;i<d.length;i++){
						if('${bldgrsdnsBean.PG_PK_CODE}' != '' && '${bldgrsdnsBean.PG_PK_CODE}' == d[i].PG_PK_CODE){
			...
			function ajaxAddUpAlert(flag){
				...
				*/
				...
				if(tSTART_NUM.length >0 && tSUFFIX==''){
			...
			function ajaxAddUp(flag){
				...
				if(flag=='Add'){
					var url = '${ctx}/address/b04cud02addrconfig!addbldgrsdns.action';
				}else{
					var url = '${ctx}/address/b04cud02addrconfig!upbldgrsdns.action';
				}
								...
								<td>
									<div id="querydiv">
										<table class="listtable" id="viewquery_table"  width="100%">
											...
											<tr  id="tr_TOTAL_NAME" style="display:none">
												<td class="listtablehead" colspan="4">
													&nbsp;${TOTAL_DISTRICT_NAME}${TOTAL_STREET_NAME}
												</td>
											</tr>
											<tr id="tr_PROVINCE_NAME" style="display:none">
											...
											<tr id="tr_STRT_ID" style="display:none">
												<td class="listtablehead" width="100">
													街道:
												</td>
												<td class="js_left_txt" id="td_STRT_ID" colspan="3">
													<select class="simple" id="STRT_ID" style="width:130">
													</select>
											...
											<tr id="tr_DORPLT_ID">
												...
												<td class="js_left_txt" id="td_DORPLT_ID" colspan="3">
													...
													<tr id="tdSTRT2_NAME">
														...
														<td class="js_left_txt" id="td_city_content" colspan="3">
															<input type="text" name="START_NUM" style="width: 200px; height:20px;" id="START_NUM" class="ac_input" size="300"  value="${bldgrsdnsBean.START_NUM}" onFocus="this.select();" onKeyUp="shNUM_FLAG('abbr')"/><font color="#FF0000">&nbsp;(有门牌需要维护门牌)</font>
														</td>
													</tr>
													<tr  id="tr_END_NUM"  style="display:none">
														...
														<td class="js_left_txt" id="td_city_content" colspan="3">
															<input type="text" name="END_NUM" style="width: 200px; height:20px;" id="END_NUM" class="ac_input" size="300"  value="${bldgrsdnsBean.END_NUM}" onFocus="this.select();" onChange="shNUM_FLAG('abbr')"/>
														</td>
													</tr>
													<tr id="tdSTRT4_NAME">
														<td class="listtablehead">
											...
											<tr id="tr_DORPLT_ID">
												...
												<td class="js_left_txt" id="td_POST_CD" colspan="3">
													<input type="text" name="POST_CD" style="width: 300px; height:20px;" id="POST_CD" class="ac_input" size="300" value="${bldgrsdnsBean.POST_CD}" onFocus="this.select();" maxlength="6"/>
												</td>
											</tr>
											<tr id="tr_DORPLT_ID">
												<td class="listtablehead" width="80">
