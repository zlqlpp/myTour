...
<html>
	...
	<head>
		...
		var bro=$.browser;
			...
			if(tSSLX.indexOf("vod2sr")>-1){
					$("#yjd").text("查询中...");
				}else if(tSSLX.indexOf("vod3nr")>-1){
					$("#wjd").text("查询中...");
				}
			...
			function callbackajaxQueryADN(data){
				...
				var str = d.split('#');
				if(str[1].indexOf("vod2sr")>-1){
					$("#yjd").text("");
					$("#yjd").text(str[2]);
				}else if(str[1].indexOf("vod3nr")>-1){
					$("#wjd").text("");
					$("#wjd").text(str[2]);
				}
			...
			function ajaxQueryPage(pageNo){
				...
				var url = '${ctx}/address/b04r02addrquery!queryorgandius.action';
			    var params = {
								PROVINCE_ID:tPROVINCE_ID,
								CITY_ID:tCITY_ID,
								COUNTY_ID:tCOUNTY_ID,
			...
			function addquerytable(data){
				...
				if(resultdata.length>0){
					for(var i=0;i<resultdata.length;i++){
						...
						tr+="</td>";
						tr+="<td align=left class=listtablebodyleft1 width=100> <a href='#' onClick='upOrganization("+ resultdata[i].DIST_CD + "," + resultdata[i].ORG_ID + ");'>维护</a> <a href='#' onClick='delOrganization("+ resultdata[i].DIST_CD + "," + resultdata[i].ORG_ID + ");'>删除</a></td>";
			...
			function upOrganization(tDIST_CD,tORG_ID){
				value = window.showModalDialog("${ctx}/address/b04cud01addrview!addorgandius.action?FLAG=ADD&DIST_CD=" + tDIST_CD + "&ORG_ID=" + tORG_ID,"Code","dialogWidth:600px;dialogHeight:600px;resizable=1,scrollbars=auto");
				backfresh(value);
			}
			function delOrganization(tDIST_CD,tORG_ID){
				$.prompt('该操作将删除此机构！确认执行操作？',{
					...
					callback: function(v){
						if(v){
							...
							var url = '${ctx}/address/b04cud02addrconfig!delorganizationdius.action';
							if(!(bro.msie && bro.version >=8)){
								$.blockUI({ message: '<img src="${ctx}/widgets/jquery/blockui/images/loadingAnimation.gif" />',css:{width:'200', border:'0'}});
							}else{
								$("#nodata").show();
				...
				$.prompt('该操作将删除地址！',{
					...
					callback: function(v){
						if(v){
							...
							var url = '${ctx}/address/b04cud02addrconfig!delorgdiustssq.action';
							if(!(bro.msie && bro.version >=8)){
								$.blockUI({ message: '<img src="${ctx}/widgets/jquery/blockui/images/loadingAnimation.gif" />',css:{width:'200', border:'0'}});
							}else{
								$("#nodata").show();
