	...
	<head>
		...
		var bro=$.browser;
			...
			function ajaxQueryPage(pageNo){
				...
				var url = '${ctx}/disp/b07r02dispquery!querytjprb.action';
			    var params = {
			...
			function addquerytable(data){
				...
				if(resultdata.length>1){
					...
					for(var i=0;i<resultdata.length;i++){
						...
						tr+="<td valign=top align=left class=selflisttablebodyleft1  width=100>"+vPROVINCE_NAME+"</td>";

						vTDQNUM = vTDQNUM + parseInt(resultdata[i].NUM);
						
						if(vPROVINCE_NAME == '全国' || vPROVINCE_NAME == '合计'){
							tr+="<td valign=top align=left class=selflisttablebodyleft1  width=100>"+vTDQNUM+"</td>";
							}
						else{
							tr+="<td valign=top align=left class=selflisttablebodyleft1  width=100>"+resultdata[i].NUM+"</td>";
							}
						
						tr+="<td valign=top align=left class=selflisttablebodyleft1  width=100>"+resultdata[i].TOTAL_COUNT+"</td>";
						tr+="<td valign=top align=left class=selflisttablebodyleft1  width=100>"+resultdata[i].DISTRI_COUNT+"</td>";
						tr+="<td valign=top align=left class=selflisttablebodyleft1  width=100>"+resultdata[i].ADDR_COUNT+"</td>";
						tr+="<td valign=top align=left class=selflisttablebodyleft1  width=100>"+resultdata[i].TDQ_COUNT+"</td>";
						tr+="<td valign=top align=left class=selflisttablebodyleft1  width=100>"+resultdata[i].ADDRTDQ_COUNT+"</td>";
						tr+="<td valign=top align=left class=selflisttablebodyleft1  width=100>"+resultdata[i].NOADDRTDQ_COUNT+"</td>";
						vDT_SJ_CODELV = resultdata[i].TDQ_COUNT / resultdata[i].TOTAL_COUNT;
						vDT_SJ_CODELV = Math.round ( vDT_SJ_CODELV * 10000 ) / 100;
						tr+="<td valign=top align=left class=selflisttablebodyleft1  width=100>"+vDT_SJ_CODELV+"%</td>";
								
						vDT_SJ_CODELVDZ = resultdata[i].ADDRTDQ_COUNT / resultdata[i].ADDR_COUNT;
						vDT_SJ_CODELVDZ = Math.round ( vDT_SJ_CODELVDZ * 10000 ) / 100;
						tr+="<td valign=top align=left class=selflisttablebodyleft1  width=100>"+vDT_SJ_CODELVDZ+"%</td>";
						...
						tr+="<td valign=top align=left class=selflisttablebodyleft2  width=100>"+vPG_SJ_CODELVQ+"%</td>";

						tr+="<td valign=top align=left class=selflisttablebodyleft2  width=100>"+resultdata[i].SRDIPQ_COUNT+"</td>";
						...
						tr+="<td valign=top align=left class=selflisttablebodyleft2  width=100>"+vSRDIPQ_COUNTLVQ+"%</td>";
					...
					var url="<tr><td colspan=14 class=selflisttablebodyleft2>" +  sj  + "</td></tr>";
					
					$('#address_table').append(url);
					
					url="<tr><td colspan=14 class=selflisttablebodyleft2>" + data.page.ajaxUrl  + "</td></tr>";
					
					$('#address_table').append(url);
					
				}else{
					var url="<tr><td colspan=14 class=selflisttablebodyleft2>找到 0 条记录</td>";
					$('#address_table').append(url);
				}
				$.unblockUI();
				$("#nodata").hide();
						...
						<table>
							<tr>
								<td>
									...
									<br />
									<div id="tablelistdiv">
										<table class="selflisttable" width="1400"
											id="address_table" cellpadding="0" cellspacing="0">
											<thead>
												<th class="selflisttableth" align="center" colspan="14">
													全名址信息匹配率(按省)
												</th>
											</tr>
											<thead>
