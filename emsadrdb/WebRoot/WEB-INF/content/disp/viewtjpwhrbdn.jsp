...
<html>
	...
	<head>
		<title>进口未匹配投递区（段）邮件名址维护率(按省)</title>
		...
		var bro=$.browser;
			...
			function ajaxQueryPage(pageNo){
				...
				var url = '${ctx}/disp/b07r02dispquery!querytjpwhrbdn.action';
			    var params = {
			...
			function addquerytable(data){
				...
				if(resultdata.length>1){
					...
					for(var i=0;i<resultdata.length;i++){
						...
						tr+="<td valign=top align=left class=selflisttablebodyleft1  width=50>"+vPROVINCE_NAME+"</td>";

						vTDQNUM = vTDQNUM + parseInt(resultdata[i].NUM);
						
						if(vPROVINCE_NAME == '全部' || vPROVINCE_NAME == '合计'){
						...
						tr+="<td valign=top align=left class=selflisttablebodyleft1  width=250>"+resultdata[i].ADDR_SR_COUNT+"</td>";

						tr+="<td valign=top align=left class=selflisttablebodyleft1  width=200>"+resultdata[i].DRWH_COUNT+"</td>";

						vPG_SJ_CODELVQ = 100;
						if(resultdata[i].ADDR_SR_COUNT > 0){
						vPG_SJ_CODELVQ = resultdata[i].DRWH_COUNT / parseInt(resultdata[i].ADDR_SR_COUNT);
						vPG_SJ_CODELVQ = Math.round ( vPG_SJ_CODELVQ * 10000 ) / 100;
						}
						tr+="<td valign=top align=left class=selflisttablebodyleft2  width=150>"+vPG_SJ_CODELVQ+"%</td>";

						//tr+="<td valign=top align=left class=selflisttablebodyleft1  width=150>"+resultdata[i].SR_COUNT+"</td>";
						
						//tr+="<td valign=top align=left class=selflisttablebodyleft1  width=100>"+resultdata[i].QBWH_COUNT+"</td>";

						//vPG_SJ_CODELVQB = 100;
						//if(resultdata[i].SR_COUNT > 0){
						//	vPG_SJ_CODELVQB = resultdata[i].QBWH_COUNT / parseInt(resultdata[i].SR_COUNT);
						//	vPG_SJ_CODELVQB = Math.round ( vPG_SJ_CODELVQB * 10000 ) / 100;
						//}
						//tr+="<td valign=top align=left class=selflisttablebodyleft2  width=150>"+vPG_SJ_CODELVQB+"%</td>";
										...
										<table class="selflisttable" width="1000"
											...
											<thead>
												<th class="selflisttableth" align="center" colspan="11">
													进口未匹配投递区（段）邮件名址维护率(按省)
												</th>
											</tr>
											<thead>
												<td align="left" class="selflisttablebodyleft1" width="50">
													地市
												</td>
												<td align="left" class="selflisttablebodyleft1" width="100">
													投递区维护数量
												</td>
												<td align="left" class="selflisttablebodyleft1" width="200">
													当日未匹配投递区邮件数
												</td>
												<td align="left" class="selflisttablebodyleft1" width="250">
													次日一点前进口及时维护投递区邮件数
												</td>
												<td align="left" class="selflisttablebodyleft2" width="200">
													进口未匹配邮件及时调度维护率
												</td>
											</thead>
										</table>
