	...
	<head>
		...
		<script type="text/javascript">
			...
			function callbackdispd(data){
				$('#DISP_OFFICE_CODET option').remove();
				var d=data.dispBeans;
				for(var i=0;i<d.length;i++){
					$("#DISP_OFFICE_CODET").append("<option value='" + d[i].DISP_OFFICE_CODE + "'>" + d[i].DISP_OFFICE_ABBR + "</option>"); 
				}
			}
			...
			function ajaxAddUp(flag){
				var tDISP_OFFICE_CODE='${DISP_OFFICE_CODE}';
				var tDISP_OFFICE_CODET=$("#DISP_OFFICE_CODET").val();
				if(tDISP_OFFICE_CODE == tDISP_OFFICE_CODET){
					$.prompt('经转局名未改变!',{buttons: {'确定': true}});
					return false;
				}
				var params = {
						DISP_OFFICE_CODE:tDISP_OFFICE_CODE,
						DISP_OFFICE_CODET:tDISP_OFFICE_CODET
	            };
				var url = '${ctx}/disp/b07cud02dispconfig!zhdisp.action';
										...
										<table class="listtable" id="viewquery_table"  width="100%">
											...
											<tr id="disps_add">
												...
												<td class="js_left_txt" id="td_privince_content" colspan="3">
													...
													<select class="simple" id="DISP_OFFICE_CODET"  style="width:100"></select>
												</td>
											</tr>
										</table>
									</div>
